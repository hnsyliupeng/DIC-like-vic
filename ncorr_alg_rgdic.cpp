...
private:
    ...
    // Methods
	void precompute();
	void analyzepoint(std::list<std::vector<double > > &queue,const int &x,const int &y,const std::vector<double> &paramvector_init,const int &num_region,const int &num_thread);
...
class_rgdic::class_rgdic(mxArray *plhs[ ],const mxArray *prhs[ ]) {
	...
	if (reference[0].gs.width == roi[0].mask.width && 
		...
		total_threads = mxGetM(prhs[3]);
		total_region = mxGetN(prhs[3]);
        
		// OpenMP Setup -------------------------------------------------//
        #ifdef NCORR_OPENMP
            // Display if openmp is supported by compiler ---------------//
            mexPrintf("Openmp supported by compiler\n");
	...
	*outstate = (double)CANCELLED; 
	...
	{       
		...
		for (int i=0; i<total_region; i++) {		
			...
			// Initialize queue
			std::list<std::vector<double> > queue;
			...
			queue.push_back(paramvector_seed);
			...
			while (queue.size() > 0) {
				...
				// Step 1: load
				std::vector<double> paramvector_init = queue.back();
...
void class_rgdic::analyzepoint(std::list<std::vector<double > > &queue,const int &x,const int &y,const std::vector<double> &paramvector_init,const int &num_region,const int &num_thread) {
	...
	if (x >= roi[0].region[num_region].leftbound &&
		...
		if (outstate_calcpoint == SUCCESS && 
			...
			// Merge paramvector based on correlation coefficient - Sort descending
			if (queue.size() == 0 || paramvector[8] < queue.back()[8]) {
				// These are edge cases, either the queue is empty or the new paramvector has the lowest corrcoef
				queue.push_back(paramvector);
			} else {
				// In this case, the paramvector must be inserted
				for (std::list<std::vector<double> >::iterator it=queue.begin(); it!=queue.end(); it++) {
					if (paramvector[8] >= (*it)[8]) {
						// Insert paramvector here
						queue.insert(it,paramvector);
						break;
					}
				}
			}	
