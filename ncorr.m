...
% Modified by: Peng Liu
% 01/02/2015,Changsha,China
%
%-------------------------------------------------------------------------%
%-------------------------------------------------------------------------%
%
        ...
        function constructor(obj)                        
            ...
            obj.data_dic = struct('displacements',struct('plot_u_dic',{}, ...                                                % U plot after DIC (can be regular or eulerian)
                                  ...
                                  'strains',struct('plot_exx_ref_formatted',{}, ...                                          % Exx Green-Lagragian strain plot
                                                   ...
                                                   'plot_evon_ref_formatted',{}, ...                                         % Evon Green-Lagragian strain plot
                                                   ...
                                                   'plot_evon_cur_formatted',{}, ...                                        % Evon Eulerian-Almansi strain plot 
    ...
    methods(Access = private)
        ...
        % displacements have been formatted.
            ...
            for i = 0:length(obj.current)-1
                ...
                strains_template(i+1).plot_evon_ref_formatted = sqrt(2/3*(strains_template(i+1).plot_exx_ref_formatted.^2+strains_template(i+1).plot_eyy_ref_formatted.^2)+...
                                                                     1/3*strains_template(i+1).plot_exy_ref_formatted.^2); %#ok<AGROW>
%                 strains_template(i+1).plot_exx_ref_formatted
%                 strains_template(i+1).plot_evon_ref_formatted
                ...
                strains_template(i+1).plot_evon_cur_formatted = sqrt(2/3*(strains_template(i+1).plot_exx_cur_formatted.^2+strains_template(i+1).plot_eyy_cur_formatted.^2)+...
                                                                     1/3*strains_template(i+1).plot_exy_cur_formatted.^2); %#ok<AGROW>
...
%             end
            obj.data_dic.strains = strains_template;
        ...
        % strain plots, or all the plots.                 
            ...
            if (strcmp(closetype,'strains') || strcmp(closetype,'all'));  
                ...
                for i = 0:length(handles_plot)-1
                    type_plot = getappdata(handles_plot(i+1),'type_plot');  
                    if (strcmp(type_plot,'exx') || strcmp(type_plot,'exy') || strcmp(type_plot,'eyy') || strcmp(type_plot,'evon'))
                        handles_plot_close = horzcat(handles_plot_close,handles_plot(i+1)); %#ok<AGROW>
                    end                    
                end
        ...
        % This function updates menu items depending on what is loaded
            ...
            if (~isempty(obj.reference) && ~isempty(obj.current) && ~isempty(obj.data_dic.dispinfo)&& ~isempty(obj.data_dic.displacements) &&  ...
                ...
                set(obj.handles_gui.topmenu_viewstrainplot_evon,'Enable','on'); 
                ...
                set(obj.handles_gui.topmenu_viewstrainplot_evon,'Enable','off'); 
        ...
        % This function checks the matlab compatibility. 
            ...
            % toolbox            
            v = ver;
            if (isempty(strfind([v.Name],'Image Processing Toolbox')) || isempty(strfind([v.Name],'Statistics Toolbox')))
                h_error = errordlg('Requires the image processing and statistics toolbox.','Error','modal');
                uiwait(h_error);                
                return;
            end
            
            % Check to see if OS is Linux or PC --------------------------%
            if (~ispc && ~isunix)
                h_error = errordlg('Ncorr has only been developed for PC and Linux.','Error','modal');
                uiwait(h_error);                
                return;
            end
        ...
        % Check to see if mex files exist and loads ncorr_installinfo ----%
            ...
            if (compile)                
                % One of the mex files is missing - tell user that ncorr
                % will recompile files.
                e = msgbox('One of the mex files is missing. If using automatic installation, then mex files will compile; if using manual installation, please close Ncorr and make sure all the files compiled properly.','WindowStyle','modal'); 
                uiwait(e);
            end
            ...
            %-------------------------------------------------------------%
            % COMMENT THIS SECTION OUT AND MANUALLY COMPILE IF            % 
            % COMPILATION FAILS!!! THIS SECTION WILL ONLY SUPPORT OPENMP  %
            % FOR COMPILERS (CL AND GCC) WHICH WERE INSTALLED THE         %
            % STANDARD WAY (mex -setup) AND THROUGH GNUMEX. IF THE        %
            % COMPILER WAS MANUALLY INSTALLED A DIFFERENT WAY, OPENMP     %
            % WILL PROBABLY NOT COMPILE CORRECTLY, ALTHOUGH SINGLE        %
            % THREADED INSTALLATION SHOULD BE FINE. INSTRUCTIONS FOR      %
            % MANUAL INSTALLATION OF MEX FILES ARE AVAILABLE IN THE       %
            % MANUAL ON ncorr.com. THESE INSTRUCTIONS INVOLVE EITHER      %
            % COMPILING THROUGH THE MATLAB TERMINAL OR MODIFYING THE      %
            % MEXOPTS.SH OR MEXOPTS.BAT FILE. BASICALLY, YOU HAVE TO      %
            % FIGURE OUT WHICH COMPILER FLAGS TO PASS FOR OPENMP TO       %
            % COMPILE PROPERLY.                                           %
            % ------------------------------------------------------------%
            
            try       
                % Errors are invoked in compile functions if files are not
                % found or if compilation fails.
                if (compile)
                    % Check if ncorr.m is in current directory  
                    listing = dir;            
                    if (any(strcmp('ncorr.m',{listing.name})))   
                        % Query user if openmp support exists. If the user 
                        % claims support exists and it really doesnt, then
                        % files generally won't compile and an error message 
                        % will be generated. In the case where the program 
                        % compiles and support does not exist, Openmp will 
                        % "run" but will return truncated results since 
                        % only one thread will compute properly.  The user 
                        % will then have to reinstall ncorr and turn this 
                        % feature off or manually set the number of threads 
                        % to 1. This code will determine which compiler is 
                        % present (or query if it is installed through 
                        % gnumex) and provide the correct compiler flags to 
                        % provide openmp upport.
                        
                        % Generally, if the flags aren't recognized, 
                        % compilation will fail, because the compiler won't 
                        % be able to recognize functions related specifically 
                        % to openmp. This is actually a good scenario because 
                        % it generates an exception which is handled by 
                        % prompting an error message, which is then followed 
                        % by a force close of ncorr.

                        % Ask user if openmp support exists, and also if 
                        % they installed with gnumex
                        [support_openmp_prelim,total_cores_prelim,compiler_gnumex_prelim,outstate_install] = gui_install(get(obj.handles_gui.figure,'OuterPosition'));
                        ...
                        if (support_openmp_prelim)
                            ...
                            % Make sure this info is valid; GNUMEX causes
                            % the above function to return empty. 
                            if ((isfield(struct(info_comp),'Details') && isfield(struct(info_comp.Details),'CompilerExecutable')) || ...
                                ~isempty(compiler_gnumex_prelim))
                                % Get compiler
                                if (isempty(compiler_gnumex_prelim))
                                    % Automatic
                                    compiler = info_comp.Details.CompilerExecutable;
                                else
                                    % Specified as g++ through gnumex
                                    compiler = compiler_gnumex_prelim;
                                end
                                
                                if (strcmp(compiler,'cl'))
                                    % This is Microsoft visual studio
                                    % Visual studio 8.0+ (2005+) supports Openmp
                                    % Openmp flag is /openmp
                                    flags_f = horzcat(flags_f{:},{'COMPFLAGS="$COMPFLAGS'},{'/openmp"'},{'COMPFLAGS="$COMPFLAGS'},{'/DNCORR_OPENMP"'});
                                elseif (strcmp(compiler,'g++'))
                                    % This is the GNU compiler; can be
                                    % windows or linux. For windows, g++ is
                                    % assumed set through gnumex since
                                    % matlab does not explicitly support
                                    % gnu on windows. 
                                    if (ispc)
                                        ...
                                        flags_f = horzcat(flags_f{:},{'COMPFLAGS="$COMPFLAGS'},{'-fopenmp"'},{'COMPFLAGS="$COMPFLAGS'},{'-DNCORR_OPENMP"'},{'GM_ADD_LIBS="$GM_ADD_LIBS'},{'-lgomp"'});
                                    elseif (isunix)
                                        % GCC 4.2+ supports openmp
                                        % NOTE: GCC must manually link 
                                        % against -lgomp. -lgomp also has 
                                        ...
                                        % correctly. 
                                        % openmp flag is -fopenmp
                                        flags_f = horzcat(flags_f{:},{'CXXFLAGS="\$CXXFLAGS'},{'-fopenmp"'},{'CXXFLAGS="\$CXXFLAGS'},{'-DNCORR_OPENMP"'},{'CXXLIBS="\$CXXLIBS'},{'-lgomp"'});
                                    end
                                else
                                    % C++ compiler is set up, but was not
                                    % recognized as either gcc or cl
                                    h_error = errordlg('C++ compiler is set up in mex, but was not recognized as GNU or Visual Studio. Either comment out the compile section in ncorr and manually compile the mex functions, or restart ncorr and disable openmp support.','Error','modal');
                                    uiwait(h_error);
        ...
        function handles_gui = init_gui(obj)
            ...
            handles_gui.topmenu_viewstrainplot_evon = uimenu( ...
                'Parent', handles_gui.topmenu_viewstrainplot, ...
                'Tag', 'topmenu_viewstrainpplot_evon', ...
                'Label', 'Evon', ...
                'Checked', 'off', ...
                'Callback', ncorr_util_wrapcallbacktrycatch(@(hObject,eventdata) callback_topmenu_viewplot(obj,hObject,eventdata,{'evon'}),handles_gui.figure), ...
                'Enable', 'off', ...
                'Interruptible','off');
            ...
            handles_gui.topmenu_viewstrainplot_all = uimenu( ...
                ...
                'Label', 'All', ...
                'Checked', 'off', ...
                'Callback', ncorr_util_wrapcallbacktrycatch(@(hObject,eventdata) callback_topmenu_viewplot(obj,hObject,eventdata,{'exx','exy','eyy','evon'}),handles_gui.figure), ...
                'Enable', 'off', ...
                'Interruptible','off');
...
function [support_openmp,total_cores,compiler_gnumex,outstate] = gui_install(pos_parent)
...
%   compiler_gnumex - string; specifies manually if g++ was installed
%   through gnumex
%   outstate - integer; returns either out.cancelled, out.failed, or
%   out.success.
    ...
    function constructor                       
        ...
        setappdata(handles_gui.figure,'val_checkbox_specifycompiler',false); 
        setappdata(handles_gui.figure,'val_checkbox_gnu',false); 
    ...
    function callback_checkbox_specifycompiler(hObject,eventdata) %#ok<INUSD>
        % Get data
        val_checkbox_gnu = getappdata(handles_gui.figure,'val_checkbox_gnu');
        
        val_checkbox_specifycompiler = get(handles_gui.checkbox_specifycompiler,'Value');     
        if (~val_checkbox_specifycompiler)
            % Set checkbox for gnu to false as well
            val_checkbox_gnu = false;
        end
        
        % Set data
        setappdata(handles_gui.figure,'val_checkbox_specifycompiler',val_checkbox_specifycompiler);
        setappdata(handles_gui.figure,'val_checkbox_gnu',val_checkbox_gnu);
        
        update_sidemenu();
    end

    function callback_checkbox_gnu(hObject,eventdata) %#ok<INUSD>        
        val_checkbox_gnu = get(handles_gui.checkbox_gnu,'Value');
    ...
    function callback_button_finish(hObject,eventdata) %#ok<INUSD>
        ...
        if (val_checkbox_gnu)
            compiler_gnumex_prelim = 'g++';
        else
            compiler_gnumex_prelim = '';
        end
        
        % Store
        total_cores = total_cores_prelim;
        support_openmp = support_openmp_prelim;
        compiler_gnumex = compiler_gnumex_prelim;
        outstate = out.success;       
    ...
    function update_sidemenu(hObject,eventdata) %#ok<INUSD>
        ...
        val_checkbox_specifycompiler = getappdata(handles_gui.figure,'val_checkbox_specifycompiler');
        val_checkbox_gnu = getappdata(handles_gui.figure,'val_checkbox_gnu');
        total_cores_prelim = getappdata(handles_gui.figure,'total_cores_prelim');
        
        % If linux then disable the gnumex option
        if (isunix)
            set(handles_gui.checkbox_specifycompiler,'enable','off');
        end
        
        set(handles_gui.checkbox_openmp,'Value',val_checkbox_openmp);
        set(handles_gui.checkbox_specifycompiler,'Value',val_checkbox_specifycompiler);                
        set(handles_gui.checkbox_gnu,'Value',val_checkbox_gnu);
        ...
        if (val_checkbox_specifycompiler)
            set(handles_gui.checkbox_gnu,'enable','on');
        else
            set(handles_gui.checkbox_gnu,'enable','off');
        end        
    ...
    function handles_gui = init_gui()
        handles_gui.figure = figure( ...
            'Tag', 'figure', ...
            'Units', 'characters', ...
            'Position', ncorr_util_figpos(pos_parent,[13 70]), ...
            'Name', 'Openmp support', ...
            'MenuBar', 'none', ...
            'NumberTitle', 'off', ...
            'Resize', 'off', ...
        ...
        handles_gui.text_openmp = uicontrol( ...
            ...
            'Style', 'text', ...
            'Units', 'characters', ...
            'Position', [1.5 8.6 66.7 2.2], ...
            'HorizontalAlignment','left', ...
            'String', 'Requires multicore CPU and Visual Studio 2005+ or GCC 4.2+ installed through mex.', ...
            'Value', 0, ...
            'Interruptible','off');
        ...
        handles_gui.checkbox_openmp = uicontrol( ...
            ...
            'Style', 'checkbox', ...
            'Units', 'characters', ...
            'Position', [1.5 11.0 66.7 1.3], ...
            'String', 'Openmp support', ...
            'Value', 0, ...
            'Callback', ncorr_util_wrapcallbacktrycatch(@callback_checkbox_openmp,handles_gui.figure), ...
            'Interruptible','off');

        handles_gui.checkbox_specifycompiler = uicontrol( ...
            'Parent', handles_gui.figure, ...
            'Tag', 'checkbox_specifycompiler', ...
            'Style', 'checkbox', ...
            'Units', 'characters', ...
            'Position', [1.5 4.9 66.7 1.3], ...
            'String', 'Manually specify GNU compiler (if installed with GNUMEX)', ...
            'Value', 0, ...
            'Callback', ncorr_util_wrapcallbacktrycatch(@callback_checkbox_specifycompiler,handles_gui.figure), ...
            'Interruptible','off');

        handles_gui.checkbox_gnu = uicontrol( ...
            'Parent', handles_gui.figure, ...
            'Tag', 'checkbox_gnu', ...
            'Style', 'checkbox', ...
            'Units', 'characters', ...
            'Position', [10.1 3.1 22 1.3], ...
            'String', 'G++ (GNUMEX)', ...
            'Value', 0, ...
            'Callback', ncorr_util_wrapcallbacktrycatch(@callback_checkbox_gnu,handles_gui.figure), ...
            'Enable','off', ...
            'Interruptible','off');
