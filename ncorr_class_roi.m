...
% This is the class definition for the region of interest.
    ...
    methods(Access = public)
        ...
        % currently not being used.
            ...
            % Find nodewidth_max_buffer - this is the maximum width
            % of the nodelist.
            nodewidth_max_buffer = 0;
            for i = 0:length(obj.region)-1  
                if (size(obj.region(i+1).nodelist,2) > nodewidth_max_buffer)
                    nodewidth_max_buffer = size(obj.region(i+1).nodelist,2);
                end
            end

            % Initialize cirroi nodelist and noderange
            cirroi_template.region(1).nodelist = -ones(cirroi_template.radius*2+1,nodewidth_max_buffer);     
