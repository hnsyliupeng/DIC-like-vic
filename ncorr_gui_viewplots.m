...
% dialogue and return.
    ...
    function constructor()                           
        ...
        setappdata(handles_gui.figure,'handle_point_max',-0.6);
        setappdata(handles_gui.figure,'handle_point_min',0.6);
...
%             set(handles_gui_sub.edit_scalebarlength,'Enable','on','backgroundcolor',[0 0 0]);
    ...
    function update_axes(action,handles_gui_sub)
        ...
        if (strcmp(action,'set') || strcmp(action,'save'))
            ...
            if (strcmp(action,'set') || (strcmp(action,'save') && getappdata(handles_gui_sub.figure,'includeinfo')))   
                ...
                if (strcmp(type_plot_sub,'exx') || strcmp(type_plot_sub,'exy') || strcmp(type_plot_sub,'eyy')|| strcmp(type_plot_sub,'evon'))
                ...
                elseif (strcmp(type_plot_sub,'exx') || strcmp(type_plot_sub,'exy') || strcmp(type_plot_sub,'eyy')|| strcmp(type_plot_sub,'evon'))
            ...
            % strain tensor name
            if (~strcmp(action,'save') && (strcmp(type_plot_sub,'exx') || strcmp(type_plot_sub,'exy') || strcmp(type_plot_sub,'eyy') || strcmp(type_plot_sub,'evon')))
                ...
                elseif (strcmp(type_plot_sub,'eyy'))
                    group_title = 'Eyy ';
                 elseif (strcmp(type_plot_sub,'evon'))
                    group_title = 'Evon ';
                end     
                if (val_popupmenu == 1)
...
%             ncorr_util_colormap(handles_gui_sub.figure); 
            ncorr_util_colormap_3(handles_gui_sub.figure); 
        end        
    ...
    % This function creates the scalebar
        ...
        text(pos_x_text,pos_y_text,1,[num2str(scalebarlength) ' ' data_dic.dispinfo.units],'parent',handle_scalebar,'color','r', ...
    ...
    % This function forms the axes seen at the top left of the plot  
        ...
        % Text
        text(offset_text,pos_text,1,'Y','parent',handle_axes,'color','r', ...
             'HorizontalAlignment','center','VerticalAlignment','middle','FontWeight','bold','FontSize',size_text,'tag','text_x');
        text(pos_text,offset_text,1,'X','parent',handle_axes,'color','r', ...
        ...
        if (pos_mouse_axes(1) >= 1 && pos_mouse_axes(3) >= 1 && pos_mouse_axes(1) <= size(roi_data.mask,2) && pos_mouse_axes(3) <= size(roi_data.mask,1))
            if (roi_data.mask(pos_mouse_axes(3),pos_mouse_axes(1)))
                ...
                elseif (strcmp(type_plot,'eyy'))
                    ...
                    units = '';
                 elseif (strcmp(type_plot,'evon'))
                    text_display = 'Evon-strain: ';
                    units = '';
                end
    ...
    % This function returns displacement/strain data in vector from the data_dic structure    
        ...
        elseif (strcmp(type_plot_sub,'eyy'))
         ...
         elseif (strcmp(type_plot_sub,'evon'))
            % Eyy:
            data = data_dic.strains(num_cur+1).(['plot_evon_' rorc '_formatted'])(data_dic.strains(num_cur+1).(['roi_' rorc '_formatted']).mask); 
        end
    end
    ...
    % This function returns plot_data from the data_dic structure    
        ...
        elseif (strcmp(type_plot_sub,'evon'))
            % Eyy:
            plot_data = data_dic.strains(num_cur+1).(['plot_evon_' rorc '_formatted']);
        end
    end
