...
// Function creates cirroi which can be accessed through the cirroi member    
    ...
    if (info_cirroi[num_thread].region.nodelist.value != NULL && info_cirroi[num_thread].region.noderange.value != NULL && info_cirroi[num_thread].mask.value != NULL) {
        ...
        info_cirroi[num_thread].activelines[idx_roi_x+(idx_nodelist/2)*(info_cirroi[num_thread].region.nodelist.height)] = 0; //Set to false
        ...
        while (info_cirroi[num_thread].queue_nodelist.size() > 0) {
            ...
            if (queue_nodeindex_buffer-1 >= 0 && idx_froi-1 >= 0) {
                for (int i=0; i<region[num_region].noderange.value[idx_froi-1]; i+=2) {
                    if (info_cirroi[num_thread].activelines[idx_froi-1+(i/2)*(info_cirroi[num_thread].region.nodelist.height)] == 0) {
                        continue;
                    }
                    ...
                    if (region[num_region].nodelist.value[(idx_froi-1)+(i+1)*(region[num_region].nodelist.height)] < queue_nodelist_top_buffer) {
                        ...
                        info_cirroi[num_thread].activelines[idx_froi-1+(i/2)*(info_cirroi[num_thread].region.nodelist.height)] = 0;
        ...
        if (subsettrunc && !circ_untrunc) {
            ...
            if ((int)ceil(sqrt(val_min)) < info_cirroi[num_thread].radius) {
                ...
                int idx_space = 3;
                int idx_plus = mod_pos(idx_min+idx_space,vec_boundary.size());  
                int idx_minus = mod_pos(idx_min-idx_space,vec_boundary.size());
                ...
                for (int i=-length_filt; i<=length_filt; i++) {
                    x_plus_f += (double)vec_boundary[mod_pos(idx_plus+i,vec_boundary.size())][0];
                    x_min_f += (double)vec_boundary[mod_pos(idx_min+i,vec_boundary.size())][0];
                    x_minus_f += (double)vec_boundary[mod_pos(idx_minus+i,vec_boundary.size())][0];

                    y_plus_f += (double)vec_boundary[mod_pos(idx_plus+i,vec_boundary.size())][1];
                    y_min_f += (double)vec_boundary[mod_pos(idx_min+i,vec_boundary.size())][1];
                    y_minus_f += (double)vec_boundary[mod_pos(idx_minus+i,vec_boundary.size())][1];
                }            
                // Divide by length
